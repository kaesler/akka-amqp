...
trait ChannelPublisher { actor: ChannelActor ⇒
  ...
  when(Available) {
    case Event(mess: PublishToExchange, Some(channel) %: _) ⇒
      publishToExchange(mess, channel) match {
        case Some(seqNo) ⇒ stay() replying seqNo
        case None        ⇒ stay()
      }
    case Event(AddReturnListener(returnListener: ActorRef), Some(channel) %: _) ⇒
      channel.addReturnListener(new ReturnListener {
        def handleReturn(replyCode: Int, replyText: String, exchange: String, routingKey: String, properties: BasicProperties, body: Array[Byte]) {
          returnListener ! ReturnedMessage(replyCode, replyText, exchange, routingKey, properties, body)
        }
      })
      stay()
  }
